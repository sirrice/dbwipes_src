...
def print_clusters(pp, cols, clusters, title=''):
    ...
    sub.set_ylim(min(ys) * 0.8, max(ys) * 1.2)
    sub.set_xlim(min(xs) * 0.8, max(xs) * 1.2)
...
if __name__ == '__main__':
    ...
    def run_experiment(datasetidx, nbadresults=10):
        ...
        outname = name
        print name
        pp = PdfPages('figs/bottomup_%s.pdf' % outname)
        ...
        for perc in [0.001]:#, 0.005, 0.01]:
            ...
            bottomup = BottomUp(aggerr=aggerr,
                                err_func = err_func,
                                perc_k=perc,
                                cols=cols, 
                                parallelize=True,
            ...
            if bottomup.all_clusters and len(bottomup.all_clusters[0].bbox[0]) == 2:
                print_clusters(pp, cols, bottomup.all_clusters, title=title+' initial clusters')
